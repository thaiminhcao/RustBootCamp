...
fn exercise3() {
    ...
    while values.len() > 0 {
        ...
        for element_index in values.iter() {
            let addition_aux = element_index;
...
fn exercise4(value: u32) -> String {
    ...
    String::from(str_ref) // Return the reference to the String
}
...
fn exercise5() {
    ...
    let res = match my_map.get(&key) {
        ...
        None => {
            ...
            my_map.get(&key).unwrap() // HERE IT FAILS
        }
    };
